...
The implementation currently supports these API profiles:
L1
L2 (With batch and synchronous services, not web services)
L3:
- [Validation]( https://openeo.org/documentation/1.0/developers/profiles/api.html#validation )
- [UDF](https://openeo.org/documentation/1.0/developers/profiles/api.html#l3-udf-udf)

It also includes selected funtionalities from L4 (Above and Beyond) profile.
...
The earth observation community is increasingly using openEO process graphs to describe workflows, and relying on the available backends to generate results.
The following is a non-exhaustive list of publicly documented cases:
...
* EEA 10m crop type maps (release announcement pending) are produced fully with openEO
