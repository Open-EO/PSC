...
openEO provides valuable standardisation and abstraction of EO processing and analysis chains across different federated compute environements. The later point is especially important in the fragmented European cloud landscape. The openEO specification has been adopted by various organisations, partners or stakeholders of ESA (e.g. EC JRC, EURAC, INPE, various academic labs) and companies (e.g. VITO, Sinergise, EODC, EOX, eGEOS, Mundialis, Google Earth Engine). ESA has further invested on the development of the operational openEO platform service ([https://openeo.cloud/)](https://openeo.cloud/).) based on the openEO specification. Thanks to this, the Copernicus Data Space Ecosystem (CDSE), sucessor to the Copernicus DIAS initive, jointly implemented/managed by ESA and the European Comission, has selected the openEO specification as a core API to enable the community to process EO data and collaborate on workflows and analytics. ESA is also investing on the OGC Geodatacube API standardization process of which openEO is considered as one of the key standards. Finally at ESA various efforts on operationalising EO science and aplication results are building on openEO due to its FAIR compliant properties, this is also why the evolution of the EO Exploitation Platform Common Architecture is going to support openEO. Making openEO an OGC Community standard is a key step to foster more widespread adoption in industry and public domain and allow to extend its current reach beyond Europe.
